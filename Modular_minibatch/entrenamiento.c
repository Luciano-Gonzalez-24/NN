...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//FORWARD
		for(int batch = 0; batch < n_examples/batch_size; batch++){				// Asumiendo que batch_size divide a n_examples
			for (int layer_indx =num_layers-1;layer_indx >= 0;layer_indx--){
				int input_size = neuralnet->layers[layer_indx].input_size;
				int output_size = neuralnet->layers[layer_indx].output_size;
				for (int i = 0; i < output_size; i++){ 
					neuralnet->layers[layer_indx].grad_bias[i]=0;					
					for (int j = 0; j < input_size; j++){
						neuralnet->layers[layer_indx].grad_weights[i*input_size+j] = 0;
					}
				}
			}
			...
			for (int ex=0; ex < batch_size; ex++){
				...
				float* x = X+(batch*batch_size+ex)*neuralnet->layers[0].input_size;
				float* y = Y+(batch*batch_size+ex)*neuralnet->layers[num_layers-1].output_size;
				...
				for (int layer_indx = 0; layer_indx < num_layers; layer_indx++){
					...
					for (int i = 0; i < output_size; i++){
						...
						neuralnet->layers[layer_indx].output[i] = (layer_indx != num_layers -1)*( Relu(neuralnet->layers[layer_indx].output[i]) - neuralnet->layers[layer_indx].output[i]) +neuralnet->layers[layer_indx].output[i];
				...
				for (int layer_indx =num_layers-1;layer_indx >= 0;layer_indx--){
					...
					int output_size = neuralnet->layers[layer_indx].output_size;
					for (int i = 0; i < output_size; i++){ 
						float grad_z = grad[i]*( (layer_indx != num_layers - 1)*( Relu_prime(neuralnet->layers[layer_indx].cache.z[i]) -1 ) + 1 ); 
					...
					if ( layer_indx > 0){
						for (int j = 0; j < input_size; j++){
							...
							for (int i = 0; i < output_size; i++){
								float grad_z = grad[i];  
								grad_z *= (layer_indx != num_layers -1)* (Relu_prime(neuralnet->layers[layer_indx].cache.z[i]) - 1) + 1;
			...
			for (int layer_indx = 0; layer_indx < num_layers; layer_indx++){
				...
				int output_size = neuralnet->layers[layer_indx].output_size;
				for( int i = 0; i < output_size; i++){
					neuralnet->layers[layer_indx].bias[i] -= learning_rate*neuralnet->layers[layer_indx].grad_bias[i] / batch_size;
					for( int j = 0; j < input_size; j++){
						neuralnet->layers[layer_indx].weights[i*input_size+j]-=learning_rate*neuralnet->layers[layer_indx].grad_weights[i*input_size+j] / batch_size;
							}
						}
					}
