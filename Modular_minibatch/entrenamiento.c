  ...
  for (int ep = 0; ep < epochs; ep++) {
    ...
    for (int batch = 0; batch < (int)(n_examples / batch_size);
      ...
      for (int ex = 0; ex < batch_size; ex++) {
        ...
        float *x =
            X + (batch * batch_size + ex) * neuralnet->layers[0].input_size;
        float *y = Y + (batch * batch_size + ex) *
                           neuralnet->layers[num_layers - 1].output_size;
        ...
        for (int layer_indx = 0; layer_indx < num_layers; layer_indx++) {
          ...
          for (int i = 0; i < output_size; i++) {
            ...
            float layer_output_aux = neuralnet->layers[layer_indx].output[i];
            for (int j = 0; j < input_size; j++) {
              layer_output_aux +=
                  neuralnet->layers[layer_indx].weights[i * input_size + j] *
                  neuralnet->layers[layer_indx].input[j];
            }
            layer_output_aux += neuralnet->layers[layer_indx].bias[i];
            neuralnet->layers[layer_indx].cache.z[i] = layer_output_aux;

            layer_output_aux = (layer_indx != num_layers - 1) *
                                   (func(layer_output_aux) - layer_output_aux) +
                               layer_output_aux;

            neuralnet->layers[layer_indx].output[i] = layer_output_aux;
          }
        }
          ...
          for (int i = 0; i < output_size; i++) {
            float grad_z =
                ...
                ((layer_indx != num_layers - 1) *
                     (func_prime(neuralnet->layers[layer_indx].cache.z[i]) -
                      1) +
                 1);
