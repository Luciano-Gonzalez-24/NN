...
int NN_Layer_innit(NeuralNetwork *neuralnet, int layer_indx, int input_size,
                   int output_size) {
  if (layer_indx != 0 &&
      input_size != neuralnet->layers[layer_indx - 1].output_size) {
    printf(
        "El tamaño del output anterior no es igual a el input de esta capa \n");
    return -1;
  }
  if (layer_indx > neuralnet->num_layers - 1) {
    printf("El indice de layer  %d es mayor que la capacidad inicializada de "
           "la nn\n",
           layer_indx);
  }
  ...
  Layer->grad_weights =
      (float *)malloc(input_size * output_size * sizeof(float));
  ...
  for (int i = 0; i < output_size; i++) {
    for (int j = 0; j < input_size; j++) {
      Layer->weights[i * input_size + j] =
          ((float)drand48() - 0.5) / 10; // random entre (-0.5,0.5)
    }
    Layer->bias[i] = 0.0f;
  }
